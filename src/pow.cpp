...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params) {
    /* current difficulty formula, abosom - DarkGravity v3, written by Evan Duffield - evan@abosompay.org */
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if (pindexLast->nHeight >= params.nLastPoWBlock) {
        ...
        } else if (pindexLast->nHeight > (params.nLastPoWBlock + params.nPoSDiffAdjustRange) && pindexLast->nHeight < (params.nStartEasyBlocks)) {
            return PoSWorkRequired(pindexLast, params);
        } else if (pindexLast->nHeight >= (params.nStartEasyBlocks) && pindexLast->nHeight <= (params.nStartEasyBlocks + params.nEasyBlocksAdjRange)) {
            return PoW2PoSRequired(pindexLast, params);
        } else {
            return PoSWorkRequired(pindexLast, params);
        }

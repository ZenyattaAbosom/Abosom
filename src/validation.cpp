...
#if defined(NDEBUG)
# error "Abosom Core cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Abosom Signed Message:\n";
...
CAmount GetBlockSubsidy(int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    */
    ...
    CAmount nSuperblockPart = (nPrevHeight > consensusParams.nBudgetPaymentsStartBlock) ? nSubsidy - (25 * COIN) : 0;
                         ...
                         CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    // ABOSOM : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    CAmount expectedReward = pindex->nHeight == Params().GetConsensus().nLastPoWBlock + 5
            ?  GetBlockSubsidy(pindex->pprev->nHeight, chainparams.GetConsensus()) + 0
    ...
    if (chainActive.Height() > fFullComplianceHeight) {
        if (!IsBlockValueValid(block, pindex->nHeight, expectedReward, pindex->nMint, strError)) {
            return state.DoS(0, error("ConnectBlock(ABOSOM): %s", strError), REJECT_INVALID, "bad-cb-amount");
        }
    }
    ...
    if (chainActive.Height() > fFullComplianceHeight) {
        if (!IsBlockPayeeValid(*coinbaseTransaction, pindex->nHeight, expectedReward)) {
            return state.DoS(0, error("ConnectBlock(ABOSOM): couldn't find masternode or superblock payments"),
                             REJECT_INVALID, "bad-cb-payee");
        }
    }

    if (!ProcessSpecialTxsInBlock(block, pindex, state, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(ABOSOM): ProcessSpecialTxsInBlock for block failed with %s", FormatStateMessage(state));
    }
    // END ABOSOM

...
const char * const BITCOIN_CONF_FILENAME = "abosom.conf";
const char * const BITCOIN_PID_FILENAME = "abosomd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "abosom" is a composite category enabling all Abosom-related debug output
                if(ptrCategory->count(std::string("abosom"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AbosomCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AbosomCore
    // Mac: ~/Library/Application Support/AbosomCore
    // Unix: ~/.abosomcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AbosomCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AbosomCore";
#else
    // Unix
    return pathRet / ".abosomcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty abosom.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override abosom.conf
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Bitcoin Core") == std::string::npos) {
        ...
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2017, 2019) + "The Abosom Core developers \n";
    }
